...
import WikipediaResearchCard from "../components/common/WikipediaResearchCard.jsx";
...
const HomeWikipedia = () => {
  ...
  const isWikiUrlValid = wikiUrl && wikiUrl.includes("wikipedia.org");
  ...
  const handleWikipediaSubmit = async () => {
    console.log("handleWikipediaSubmit called with URL:", wikiUrl);
    try {
      // הקוד המקורי עם console.log הוספתי
      ...
      if (data.nodes && data.links) {
        console.log(
          "Setting network data with nodes:",
          data.nodes.length,
          "links:",
          data.links.length
        );
        setNetworkData(data);
        console.log("Network data set requested, waiting for render...");
        
      } else {
        console.warn("No valid discussion data found in the response");
        setMessage("No valid discussion data found.");
      }
    } catch (error) {
      console.error("Error fetching Wikipedia data:", error);
      setMessage("Error fetching Wikipedia data: " + error.message);
    }
  };
  ...
  return (
    <Container fluid className="upload-section">
      ...
      <WikipediaResearchCard
        ...
        handleWikipediaSubmit={handleWikipediaSubmit}
      ...
      {/* Only show filters and network visualization when network data exists */}
